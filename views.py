...
async def submit_business_acquisition(request: Request, db: Session = Depends(get_db)):
    try:
        ...
        try:
            process_submission_complete.delay(submission.id, files_data, "LOI")
            print(f"🚀 Started background processing for LOI submission {submission.id}")
        except Exception as e:
            print(f"⚠️ Failed to start background tasks: {e}")

        return templates.TemplateResponse("business_form.html", {
            "request": request,
            "success": f"Thank you {submission.full_name}\! Your submission has been received. Your LOI Questions analysis report will be sent to {submission.email} shortly.",
            "form_data": {}
        })
...
async def submit_cim_questions(request: Request, db: Session = Depends(get_db)):
    try:
        form = await request.form()
        full_name = (form.get('full_name') or '').strip()
        email = (form.get('email') or '').strip().lower()
        ...
        revenue = to_float(form.get('revenue'))
        avg_sde = to_float(form.get('avg_sde'))
        total_adjustments = to_float(form.get('total_adjustments'))
        number_of_employees = to_int(form.get('number_of_employees'))
        terms_raw = form.get('terms_accepted')
        terms_accepted = True if str(terms_raw).lower() in ['on', 'true', '1', 'yes'] else False
        ...
        submission = CIMQuestion(
            ...
            reason_for_selling=reason_for_selling,
            owner_involvement=owner_involvement,
            gm_in_place=gm_in_place,
            tenure_of_gm=tenure_of_gm,
        ...
        try:
            process_submission_complete.delay(submission.id, files_data, "CIM")
            print(f"🚀 Started background processing for CIM submission {submission.id}")
        except Exception as e:
            print(f"⚠️ Failed to start background tasks: {e}")

        return templates.TemplateResponse("cim_questions.html", {
            "request": request,
            "success": f"Thank you {submission.full_name}\! Your submission has been received. Your CIM Questions analysis report will be sent to {submission.email} shortly.",
            "form_data": {}
        })

    except Exception as e:
        db.rollback()
        print(f"Error processing CIM submission: {str(e)}")
        return templates.TemplateResponse("cim_questions.html", {
            "request": request,
            "error": "An error occurred while processing your submission.",
            "form_data": {}
